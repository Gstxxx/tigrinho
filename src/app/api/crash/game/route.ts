...
import { generateGameHash, generateCrashPoint, generateSeedFromHash } from '@/lib/crash';
...
export async function GET() {
  try {
    ...
    // Se não houver jogo pendente ou em andamento, criar um novo
    if (!latestGame) {
    ...
    const formattedBets = bets.map((bet: CrashBet & { user: { id: string, name: string } }) => ({
      id: bet.id,
      amount: bet.amount,
      status: bet.status,
