...
export async function middleware(request: NextRequest) {
  ...
  const protectedPagePaths = ["/dashboard", "/bets", "/events", "/cassino"];
  ...
  if (!token) {
    ...
    console.log(
      "Middleware - Redirecionando para login: Token ausente (Página)"
    );
  ...
  try {
    ...
    console.log(
      "Middleware - Payload:",
      payload ? `Válido (userId: ${payload.userId})` : "Inválido"
    );
    ...
    if (!payload) {
      ...
      // Para rotas de páginas, redirecionar para o login
      console.log(
        "Middleware - Redirecionando para login: Token inválido (Página)"
      );
      return NextResponse.redirect(new URL("/login", request.url));
    }
    ...
    console.log(
      "Middleware - Autenticação bem-sucedida, userId:",
      payload.userId
    );
